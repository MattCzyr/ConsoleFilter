## Version 4.0.0
#### NeoForge 26.1, 1.21.11, 1.21.1
- Updated to NeoForge 26.1
- Updated to NeoForge 1.21.11
- Updated to NeoForge 1.21.1

## Version 3.0.0
#### Fabric 26.1, 1.21.11, 1.21.1, 1.20.1
- Updated to Fabric 26.1
- Updated to Fabric 1.21.11
- Updated to Fabric 1.21.1
- Updated to Fabric 1.20.1

## Version 2.1.0
#### Forge 1.20.1
- Added support for filtering messages by logger name
- Added support for filtering messages printed to standard error
- Fixed issue with some Log4j messages not being filtered

## Version 2.0.0
#### Forge 1.20.1, 1.20, 1.19.4
- Updated to Forge 1.20.1
- Updated to Forge 1.20
- Updated to Forge 1.19.4
- Updated config to allow filtering messages by regex

## Version 1.1.1
#### Forge 1.12.2, 1.7.10
- Fix some formatted Log4j messages not being filtered

## Version 1.1.0
#### Forge 1.12.2
- Updated to Forge 1.12.2
- Added a filter for System.out

## Version 1.0.0
#### Forge 1.11, 1.10.2, 1.9.4, 1.8.9, 1.7.10
- Updated to Forge 1.11
- Updated to Forge 1.10.2
- Updated to Forge 1.9.4
- Updated to Forge 1.8.9
- Initial release for Forge 1.7.10